...
print_HTML_no_analysis <- function(parameters, cell_stats, seq_stats, sample_stats, dir, sample.name){
  ...
  system(paste0('base64 -i ', dir, '/UMI_hist_plot.png > ', dir, '/UMI_hist.txt'))
  system(paste0('base64 -i ', dir, '/Gene_hist_plot.png > ', dir, '/Gene_hist.txt'))
  system(paste0('base64 -i ', dir, '/bc_rank_plot.png > ', dir, '/barcode_rank.txt'))
...
margin: 0px;
  ...
  HTMLEndFile()
  system(paste0("sed -i.bak '/<hr/d' ", dir, "/", sample.name, "_summary.html"))
  system(paste0("sed -i.bak '/<font/d' ", dir, "/", sample.name, "_summary.html"))
  system(paste0("sed -i.bak '/R2HTML/d' ", dir, "/", sample.name, "_summary.html"))
  system(paste0("sed -i.bak 's/nequations=0;/function show(shown, hidden) {document.getElementById(shown).style.display=",'"block"',';document.getElementById(hidden).style.display="none"',";return false;}/g' ", dir, "/", sample.name, "_summary.html"))
  }
